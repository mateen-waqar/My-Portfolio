          ...
          {displayedServices.map((service, i) => {
            ...
            return (
              <Reveal key={service.id} delay={0.04 * (i % 4)}>
                <a
                  href="#contact"
                  className="group relative flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-border bg-surface/80 p-6 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-[#10b981]/50 hover:bg-surface-2 hover:shadow-[0_0_25px_-5px_rgba(16,185,129,0.2)]"
                >
                  <div>
                    {/* Top Row: Index + Icon */}
                    ...
                    <h3 className="mt-5 text-lg font-bold tracking-tight text-foreground group-hover:text-[#10b981] transition-colors">
                      {service.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex flex-wrap gap-1.5">
                        {service.features.slice(0, 2).map((feat) => (
                          <span
                            key={feat}
                            className="rounded-md border border-border/50 bg-surface-2/80 px-2 py-0.5 font-mono text-[10px] text-muted-2"
                          >
                            {feat}
                          </span>
                        ))}
